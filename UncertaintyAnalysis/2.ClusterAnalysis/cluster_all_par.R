...
RR_med <- RR_med %>%
  mutate(Release_Mm3mo = replace(Release_Mm3mo, cluster=="2" & time=="t9", 0.1 *Release_Mm3mo))
...
p <- p+ geom_line(data = Release_det_hyrum_plot, aes(x=time , y=Release_Mm3mo, group=1, color="Deterministic model"),  size=2)
...
pairs(~WASH_km2+ AquaticArea_km2+FloodplainArea_km2+WetlandsArea_km2+Demand_Mm3yr+ReachGain_Mm3yr+Reservoir_Release_Diff+Budget +rsi_centroid+Available_FloodplainArea_km2yr, data=Unc.feas, col=pam_fit$cluster,
...
clust1.Z.best <- clust1.Z %>%
  filter(Z1_Floodplain >0.3, Z1_Aquatic >0.3, Z1_Wetlands > 0.7)
...
clust1.Z.worst <- clust1.Z %>%
  filter(Z1_Floodplain <0.3, Z1_Aquatic <0.3, Z1_Wetlands < 0.4)
...
image.plot(interp_habitats_clust1, xlab="Normalized Aquatic Habitat Area", ylab="Normalized FLoodplain Habitat Area", main= "Low Habitat Quality Needs", col= rev(topo.colors(64)), cex.main=2.5, cex.axis= 2.5, cex.lab=2.5, 
...
clust2.Z.best <- clust2.Z %>%
  filter(Z2_Floodplain >0.5, Z2_Aquatic >0.5, Z2_Wetlands > 0.75)
...
clust2.Z.worst <- clust2.Z %>%
  filter(Z2_Floodplain <0.3, Z2_Aquatic <0.3, Z2_Wetlands < 0.3)
...
image.plot(interp_habitats_clust2, xlab="Normalized Aquatic Habitat Area", ylab="Normalized FLoodplain Habitat Area", main="High Habitat Quality Needs",  col= rev(topo.colors(64)), cex.main=2.5, cex.axis= 2.5, cex.lab=2.5, 
           axis.args = list(cex.axis = 1.5),
           legend.args=list(text='Normalized Wetland Habitat Area', side=1, line=2.5, cex=2.5), horizontal = TRUE)
...
#Export to txt
setwd("C:/Users/Ayman/Box Sync/USU/Thesis/GAMS/GAMSCode/Clustering/States/All_parameters")
write.table(Unc.feas.clust, "UncPar.txt", sep="\t", row.names = FALSE)
