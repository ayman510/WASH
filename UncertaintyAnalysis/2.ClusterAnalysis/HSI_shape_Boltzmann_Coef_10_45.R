...
X <- seq(0.10,0.7,length.out=n_points)  #depth
...
X_op <- seq(0.1, 0.7, length.out =10)
Y_op <- 0+((1-0)/(1+exp((0.30-X_op)/0.0344)))
...
p <- p + labs(x="Water Depth (m) for Bonneville cutthroat trout", y="Aquatic Suitability Index")
p <- p +  ylim(0,1)
p
ggsave("BCT_10_45.jpeg", path="C:/Users/Ayman/Box Sync/USU/Thesis/Second Paper")




#Write coefficients to csv file
#write.csv(nls_coef_df, file="C:/Users/Ayman/Box Sync/USU/Thesis/GAMS/GAMSCode/MonteCarlo/HSI_Shape/BCT10_45/SampleCoef/coef.csv")
